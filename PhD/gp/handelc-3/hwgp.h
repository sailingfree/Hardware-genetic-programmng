...
* Some macros
*/
extern RAM UINT32 workingPop1 [MAXPAR][MAXNODES];
extern RAM UINT32 workingPop2 [MAXPAR][MAXNODES];
...
UINT32 WorkingPopRead(Phase a, WorkIndex b, Index c) 
{
    if(a) 
        return workingPop1[b][c];
    else
        return workingPop2[b][c];
}


void WorkingPopWrite(Phase a, WorkIndex b, Index c, UINT32 v)
{
    if(a)
        workingPop1[b][c] = v;
    else
...
#define WorkingPopRead(a,b,c) (a ? workingPop1[b][c] : workingPop2[b][c])
#define WorkingPopWrite(a,b,c,v) (a ? (workingPop1[b][c]=v) : (workingPop2[b][c]=v))
