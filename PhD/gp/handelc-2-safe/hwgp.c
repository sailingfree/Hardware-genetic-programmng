...
Bool xover(WorkIndex w)
{
  ...
  PAR {
    x1 = randPC();
    len1 = ProgLen(phBreed,w1);
  }
  PAR {
    x2 = randPC();
    len2 = ProgLen(phBreed,w2);
  }
  ...
  */
  ...
  ProgLenSet(phBreed, w1, len1);
  ProgLenSet(phBreed, w2, len2);
  return 0;
}
...
INLINE Bool evalProg(Phase phase, WorkIndex indx)
{
    ...
    */
    PAR {
	    ...
	    regs[1] = curFit[1];
	pc      = START;
        len     = adju(ProgLen(phEval,indx),IDXW);
    }
    ...
    */
    if(regs[r0] == bit0 ^ bit1 ) {
      val = FitnessGet(phase,indx);
      val >>= 1;
      FitnessSet(phase,indx,val);
      if(val == 0) 
	        return 1;
        else
            delay;
...
Bool selection(void)
{
  ...
  do {
    ...
    do {
      ...
      PP1000ReadBank0(ctrl, address); 
      PP1000ReadBank0(fit1, address | 1);
    } while(InWork(ctrl));
...
Bool breed(void)
{
  ...
  do {
    ...
    w1 = ProgNumber(phBreed,count1);
    stale[count1] = adju(w1, SRAMW);
    ProgNumberSet(phBreed,count2, adju(worst1,BIT32));

    /* Ditto for 2nd individual */
    worst2=findWorst(TRUE,worst1);
    w2 = ProgNumber(phBreed,count2);
    stale[count2] =  adju(w2, SRAMW);
    ProgNumberSet(phBreed,count2, adju(worst2,BIT32));
...
Bool printProg(PopIndex idx)
{
  ...
  word = ProgNumber(phBreed,w);
  dooutdata(IND, adju(word,8));
  word = Fitness(phBreed,w,FITN);
  dooutdata(FIT, adju(word,8));
  len = adju(ProgLen(phBreed,w), IDXW);
...
{ 
	...
	*/
    SetProgram(phase, 0, proglen, progNum, DEFAULT_FITNESS);
	PAR {
    	progNum++;
    	i = START;
		icount = 0;
...
Bool writeBackInd(Phase phase, WorkIndex ind)
{
  ...
  val = WorkingPop(phase,ind,0);
  WorkingPop(phase,ind,0) = val & ~INWORK;
