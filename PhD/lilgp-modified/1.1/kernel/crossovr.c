...
int operator_crossover_init ( char *options, breedphase *bp )
{
     ...
     free_o_rama ( j, &argv );
     keep1 = get_parameter("keep_trying1");
     if(keep1) {
       cd->keep_trying1 = translate_binary(keep1);
       if(cd->keep_trying1 == -1) {
	 cd->keep_trying1 = 1;
       }
     }

     keep2 = get_parameter("keep_trying2");
     if(keep2) {
       cd->keep_trying2 = translate_binary(keep2);
       if(cd->keep_trying2 == -1) {
	 cd->keep_trying2 = 1;
       }
     }
     ...
     if ( !errors )
     {
          ...
          printf ( "   keep_trying1: %d\n", cd->keep_trying1 );
          printf ( "   keep_trying2: %d\n", cd->keep_trying2 );
...
#ifdef DEBUG_CROSSOVER
     fprintf ( stdout, "parent 1 is:\n" );
     print_individual ( oldpop->ind+p1, stdout );
     fprintf ( stdout, "parent 2 is:\n" );
     print_individual ( oldpop->ind+p2, stdout );
#endif
     ...
     while(1)
     {
	  ...
	  /** validate the first offspring against the tree node and depth
	    limits; set "badtree1" if any are violated. **/
          ...
          badtree1 = 0;
	  tree_map[t1].nodelimit = ind_nodelimit;
	  tree_map[t2].nodelimit = ind_nodelimit;
	  ...
	  /* if we're supposed to keep trying, skip up and choose new crossover
	     points. */
          if ( cd->keep_trying1 && badtree1 ) {
	    printf("Retrying cos child 1 too big (%d)\n", ns1);
	    continue;
	  }
	  ...
	  if ( cd->keep_trying1 && badtree1) {
	    //	    printf("Retrying individual1\n");
	    continue;
	  }
	  if ( cd->keep_trying2 && badtree2) {
	    //	    printf("Retrying individual2\n");
	    continue;
	  }
...
#ifdef DEBUG_CROSSOVER
               fprintf ( stdout, "offspring 1 is allowable.\n" );
#endif
...
#ifdef DEBUG_CROSSOVER
               fprintf ( stdout, "offspring 1 is too big; copying parent 1.\n" );
#endif
          }
	  ...
	  /* if the new population needs another member (it's not full) */
...
#ifdef DEBUG_CROSSOVER
                    fprintf ( stdout, "offspring 2 is allowable.\n" );
#endif
...
#ifdef DEBUG_CROSSOVER
                    fprintf ( stdout, "offspring 2 is big; copying parent 2.\n" );
#endif
               }
		  ...
		  second. */
               fprintf ( stdout, "offspring 2 not needed.\n\n" );
          }
#endif
