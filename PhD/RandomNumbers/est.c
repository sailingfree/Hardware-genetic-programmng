...
----------------------------------------------------------------------
*/
#define ALPHA 3                           /* arrays of size 2^alpha */
#define OMEGA 3           /* omega bits per value in internal state */
#define OMICRON 3                /* number of bits in values tested */
#define CORRBIT 0x1             /* which bit to use for corr values */
#define SHIFT 2                            /* amount of barrelshift */
#define CHILEN ((u4)0)     /* chisquare ignores counts smaller than */
...
#define fff(mm,x)  (*(u4 *)((char *)(mm) + ((x) & ((RANDSIZ-1)<<2))))
#define rngstep(mix,a,b,mm,m,m2,r,x) \
{ \
  x = *m;  \
  a = (a^(mix)) + *(m2++); \
  *(m++) = y = fff(mm,x) + a + b; \
  *(r++) = b = fff(mm,y>>RANDSIZL) + x; \
}
...
rng *r2;   /* not used */
{
   register u4 a,b,x,y,*m,*mm,*m2,*rr,*mend;

   mm  = r1->mm; rr = r1->rr;
   a  = r1->a;  b  = r1->b + (r1->c++);  
   for (m = mm, mend = m2 = m+(RANDSIZ/2); m<mend; )
   {
      rngstep( a<<13, a, b, mm, m, m2, rr, x);
      rngstep( a>>6 , a, b, mm, m, m2, rr, x);
      rngstep( a<<2 , a, b, mm, m, m2, rr, x);
      rngstep( a>>16, a, b, mm, m, m2, rr, x);
   }
   for (m2 = mm; m2<mend; )
   {
      rngstep( a<<13, a, b, mm, m, m2, rr, x);
...
/* XXX: Experimental RNG -- place your RNG here */
...
{
  register u4 a,b,c,i,x,y,z,w1,w2,w3,w4,w5,w6,w7,w8,*mm,*rr,*ss;
#define ind(x) ((x)&MASK)
  
  mm = rtest->mm;  rr = rtest->rr;  ss = rtest->ss;
  a = rtest->a; b = rtest->b; c = rtest->c;
  for (i=0; i<SIZE; ++i)
  {
     x = mm[i];
     a = ((a<<SHIFT)^(a>>(OMEGA-SHIFT))) & WMASK;
     a = (a+mm[ind(i+(SIZE/2))]) & WMASK;
     mm[i] = (a+b) & WMASK;
     rr[i] = b & UMASK;
     b = x;
  }
  rtest->a = a; rtest->b = b; rtest->c = c;

#ifdef NEVER    /* store deleted code inside the NEVER area */
  /* ss is part of the secret internal state */
  for (i=0; i<SIZE; ++i)  /* alleged RC4 */
  {
    x=mm[i];
    a=(a+x)&MASK;
    y=mm[a];
...
/* flow_allo() is where you decide which values and tests to use */
...
static val *flow_allo()
{
  ...
  {
    val *vt;
    if ((i != VALNORM) && (i != VALCORR) && (i != VALJUMP)) continue;
    vt = val_allo( i);
    for (j=TESTLAST; j--;)
    {
      test *t;
      if ((j != TESTGAP) && (j != TESTUNI) && 
          (j != TESTRUN) && (j != TESTMAU)) continue;
      t = test_allo( j);
      t->all.next = vt->test;
      if (i == VALNORM && j == TESTGAP) t->all.display = TDNONE;
      vt->test = t;
    }
    vt->next = v;
    v= vt;
